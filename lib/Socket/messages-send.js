...
const crypto = require("crypto"); // Added for supportPayload messageSecret
...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = false }) => {
        ...
        // [AI BADGE LOGIC v2.1.9]
        // 1. For GROUPS: Use supportPayload (safest, no drop)
        if (AI && !isPrivate && !isStatus && !isNewsletter) {
             const msgType = Object.keys(messages)[0];
             if (msgType) {
                 messages[msgType].contextInfo = messages[msgType].contextInfo || {};
                 messages[msgType].contextInfo.messageSecret = messages[msgType].contextInfo.messageSecret || crypto.randomBytes(32);
                 messages[msgType].contextInfo.supportPayload = JSON.stringify({
                     version: 1,
                     is_ai_message: true,
        ...
        await authState.keys.transaction(async () => {
            ...
            // [AI BADGE LOGIC v2.1.9]
            // 2. For PRIVATE CHATS: Use biz_bot (standard method, works fine)
            if (AI && isPrivate) {
                const botNode = {
                    tag: 'bot', 
                    attrs: {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // Auto Safe Contact (Moved from sendMessage to relayMessage or handled by caller, but keeping here for now)
            // ContactManager.checkAndSave(jid, { user: authState.creds.me });
                ...
                switch(messageType) {
                    case 'PAYMENT':
                        ...
                        return await relayMessage(jid, paymentContent, {
                            ...
                            ...getParticipantAttr(),
                            AI: true // Default to true via relayMessage logic which now handles the split
                        });
                        ...
                        content: [
                            {
                                attrs: {},
                                tag: 'support_payload',
                                content: Dugong_1.DugongMessage.fromObject(content.ai).toBuffer()
                            }
                        ]
                    });
                } else {
